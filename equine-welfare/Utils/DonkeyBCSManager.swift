...
struct DonkeyBCSBodyPart: Identifiable {
    var id: String { name }
    let name: String
...
struct DonkeyBCSBodyPartDescription: Codable {
...
struct DonkeyBCSData: Codable {
    let score: Int
    let photo: String
    let bodyParts: [String: DonkeyBCSBodyPartDescription]
    ...
    init(from decoder: Decoder) throws {
        ...
        var tempBodyParts = [String: DonkeyBCSBodyPartDescription]()
        ...
        for key in allKeys.allKeys {
            ...
            if let nestedContainer = try? allKeys.nestedContainer(keyedBy: DynamicCodingKeys.self, forKey: key) {
                ...
                if let description = try? nestedContainer.decode([String].self, forKey: DynamicCodingKeys(stringValue: "description")) {
                    let bodyPartDesc = DonkeyBCSBodyPartDescription(description: description)
...
// Using the existing DynamicCodingKeys from the project namespace

class DonkeyBCSManager {
    static let shared = DonkeyBCSManager()
    ...
    private var bcsScores: [String: DonkeyBCSData] = [:]
    ...
    private func loadBCSData() {
        guard let url = Bundle.main.url(forResource: "BCS-Donkey", withExtension: "json"),
              let data = try? Data(contentsOf: url) else {
            print("⚠️ Failed to load BCS-Donkey.json file")
            return
        }
        
        do {
            let decoder = JSONDecoder()
            bcsScores = try decoder.decode([String: DonkeyBCSData].self, from: data)
        } catch {
            print("❌ Error decoding Donkey BCS data: \(error)")
        }
    }
    ...
    func getBCSData(for score: Int) -> [DonkeyBCSBodyPart]? {
        let scoreKey = "BCS \(score)"
        
        guard let data = bcsScores[scoreKey] else {
            print("⚠️ No Donkey BCS data found for score: \(score)")
            return nil
        }
        
        // Convert the dictionary to an array of DonkeyBCSBodyPart 
        var bodyParts = data.bodyParts.map { name, description in
            DonkeyBCSBodyPart(name: name, descriptions: description.description)
        }
